...
### the spline basis function ###
# bsfun<-function(pii,r){
#   nodes <- gaussHermiteData(B)$x
#   bas<-bs(nodes, degree=1, knots=c(0.2,0.4,0.6,0.8)) 
#   result<-predict(bas,pii)[1,]
#   dim(result) <- c(r,1)
#   return(result)
# }
...
max_para_quad <- function(X, Y, theta, u, omega, quad_weights, f_theta, n, B, d, p,a,b)
{
  ...
  # temp <- fc%*%solve(t(fc)%*%fc)
  # P_FX <- temp %*% (t(fc)%*%X_tilde_weigh)
  Sigmahat <- cov(X_tilde_weigh)
  #Sigmahat <- cov(X_tilde)
...
EMquad_esti_para <- function(X, Y, d=2, r=r, B=50, iter.max =30, err.tol = 0.001){
  ...
  theta <- matrix(rep(0),ncol = B,nrow = n)
  ...
  # bas<-bs(nodes, degree=1, knots=c(0.25,0.5,0.75))
  # for(i in 1:n){
  #   for(j in 1:B){
  #     f_theta[,i,j] <- predict(bas,theta[i,j])[1,]
  #   }
  # }
  ...
  while (err2 >= err.tol & iter <= iter.max){
    ...
    # R.new <- solve(est_para$Deltahat) %*% est_para$Gammahat
    # inv2 <- solve(crossprod(R.new,R.new))
    # P2<-R.new %*% inv2 %*% t(R.new)
    # err <- sqrt(sum(diag(t(P1-P2)%*%(P1-P2))))
    # print(err)
  ...
  AIC <- -2*L_value+2*paranum
  
  #print(paste0("iter = ", iter, "; err1 = ", err1, "; err2 = ", err2))
  return(list(MU = MU, GAMMA = GAMMA, BETA  = BETA, DELTA = DELTA, L=L_value, numpar=paranum,bic=BIC,aic=AIC))
}


GHEMdata <- function(X, Y, MU.ini, Gamma.ini, BETA.ini, Delta.ini, d=2,r=r, B=50,iter.max =15, err.tol = 0.001,a.ini, b.ini){
  ...
  while (err2 >= err.tol & iter <= iter.max){
    ...
    b<-as.numeric(glm.fit$coefficients[1])
    a<-as.numeric(glm.fit$coefficients[2])
    est_para <- max_para_quad(X, Y, theta,u, omega, quad_weights, f_theta, n, B, d, p, Y_tilde,a,b)
